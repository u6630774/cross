...
modelfcn8s = torchfcn.models.FCN8s(n_class=21)
modelfcn8s.load_state_dict(torch.load( "pretrained/FCN/model_best.pth.tar")['model_state_dict'])
...
def validate(opts, model, model1, loader, device, metrics, ret_samples_ids=None):
    ...
    def get_PSNR(imagea, imageb):
        ...
        if(mse == 0): 
            return 100
        max_pixel = 255.0
        psnr = 20 * math.log(max_pixel / math.sqrt(mse))
        return psnr
...
# =============================================================================
 ...
 #            sign_data_grad = torch.sign(data_grad)
            ...
            # adversarial_x = attacks.segpgd(images,new_images,new_labels,0.03,model)
...
#            preds = outputs.detach().max(dim=1)[1].cpu().numpy()
            ...
            # metrics_1.update(targets, preds)
...
#                    Image.fromarray(adversarial_img_y).save('results/%d_atyimage.png' % img_id)
    ...
    # return score,score_1, ret_samples
...
def main():
    ...
    if opts.test_only:
        ...
        val_score, ret_samples = validate(
            opts=opts, model=model,model1=modelfcn8s, loader=val_loader, device=device, metrics=metrics, ret_samples_ids=vis_sample_id)
...
# =============================================================================
            ...
            if (cur_itrs) % opts.val_interval == 0:
                ...
                model.eval()
                val_score, ret_samples = validate(
                    opts=opts, model=model,model1=modelfcn8s, loader=val_loader, device=device, metrics=metrics,
